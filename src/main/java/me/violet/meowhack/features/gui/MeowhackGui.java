package me.violet.meowhack.features.gui;
...
import me.violet.meowhack.features.gui.items.buttons.ModuleButton;
...
public class MeowhackGui extends Screen {
    private static MeowhackGui meowhackGui;
    private static MeowhackGui INSTANCE;

    static {
        INSTANCE = new MeowhackGui();
    }

    private final ArrayList<Component> components = new ArrayList();

    public MeowhackGui() {
        super(Text.literal("Meowhack"));
        setInstance();
        load();
    }

    public static MeowhackGui getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new MeowhackGui();
        }
        return INSTANCE;
    ...
    public static MeowhackGui getClickGui() {
        return MeowhackGui.getInstance();
    }
    ...
    private void load() {
        int x = -84;
        for (final Module.Category category : Meowhack.moduleManager.getCategories()) {
            this.components.add(new Component(category.getName(), x += 90, 4, true) {

                @Override
                public void setupItems() {
                    counter1 = new int[]{1};
                    Meowhack.moduleManager.getModulesByCategory(category).forEach(module -> {
                        if (!module.hidden) {
    ...
    public void updateModule(Module module) {
        for (Component component : this.components) {
            for (Item item : component.getItems()) {
                if (!(item instanceof ModuleButton button)) continue;
